...
nnoremap  <silent> <C-h><C-h> :call <SID>Highlight("h") \| nohls<CR>
" Advance color for next line highlight
nnoremap  <silent> <C-h><C-a> :call <SID>Highlight("a")<CR>
" Go back color for next line highlight
nnoremap  <silent> <C-h><C-x> :call <SID>Highlight("x")<CR>
" Clear last line highlight
nnoremap  <silent> <C-h><C-r> :call <SID>Highlight("r")<CR>

" Highlight word under cursor (whole word match)
nnoremap  <silent> <C-h><C-w> :call <SID>Highlight("w") \| nohls<CR>
" Highlight all lines having word under cursor (whole word match)
nnoremap  <silent> <C-h><C-l> :call <SID>Highlight("l") \| nohls<CR>
" Highlight word under cursor (partial word match)
nnoremap  <silent> <C-h><C-f> :call <SID>Highlight("f") \| nohls<CR>
" Highlight all lines having word under cursor (partial word match)
nnoremap  <silent> <C-h><C-k> :call <SID>Highlight("k") \| nohls<CR>
" Highlight last search pattern
nnoremap  <silent> <C-h><C-s> :call <SID>Highlight("s") \| nohls<CR>
" Highlight all lines having last search pattern
nnoremap  <silent> <C-h><C-j> :call <SID>Highlight("j") \| nohls<CR>
" Clear last pattern highlight
nnoremap  <silent> <C-h><C-d> :call <SID>Highlight("d")<CR>

" Clear all highlights
nnoremap  <silent> <C-h><C-n> :call <SID>Highlight("n")<CR>
...
if !exists('g:lcolor_bg')
    "errors in gvim
    "let g:lcolor_bg = "darkpurple,darkseagreen,violet,lightred,lightgreen,lightblue,darkmagenta,slateblue"
    let g:lcolor_bg = "darkblue,darkseagreen,violet,lightred,lightgreen,lightblue,darkmagenta,slateblue"
endif
...
if !exists('g:lcolor_bg_cterm')
    " Given the first option a white foreground because when
    " highlighting current line, the normal current-line-background,
    " which is dark grey makes black text impossible to read
   "let g:lcolor_bg_cterm = "DarkGreen,DarkBlue,LightGreen,DarkCyan,DarkYellow,LightMagenta,LightBlue,LightGray"
   let g:lcolor_bg_cterm = "030,DarkBlue,LightGreen,DarkCyan,DarkYellow,LightMagenta,LightBlue,LightGray"
endif

if !exists('g:lcolor_fg_cterm')
   "let g:lcolor_fg_cterm = "LightRed,Black,Black,Black,LightYellow,DarkMagenta,DarkBlue,Black"
   let g:lcolor_fg_cterm = "051,Black,Black,Black,LightYellow,DarkMagenta,DarkBlue,Black"
endif

" Define colors for Pattern highlight
if !exists('g:pcolor_bg')
    "errors in gvim
    let g:pcolor_bg = "yellow,darkblue,green,darkmagenta,cyan,darkgreen,orange,darkred"
    "let g:pcolor_bg = "yellow,darkblue,green,darkmagenta,cyan,darkbrown,orange,darkred"
endif
...
if !exists('g:pcolor_bg_cterm')
   let g:pcolor_bg_cterm = "DarkGreen,DarkBlue,LightGreen,DarkCyan,DarkYellow,LightMagenta,LightBlue,LightGray"
endif

if !exists('g:pcolor_fg_cterm')
   let g:pcolor_fg_cterm = "LightRed,Black,Black,Black,LightYellow,DarkMagenta,DarkBlue,Black"
endif
