...
function! eclim#lang#hierarchy#CallHierarchy(lang, default_action, bang, ...) " {{{
  ...
  nnoremap <buffer> <silent> E :silent! call <SID>Open('edit')<cr>
  nnoremap <buffer> <silent> S :silent! call <SID>Open('split')<cr>
  nnoremap <buffer> <silent> T :silent! call <SID>Open("tablast \| tabnew")<cr>
...
function! s:Open(action) " {{{
  ...
  if info.file != ''
    ...
    let action = a:action
    silent! call eclim#util#GoToBufferWindowOrOpen(
